    ...
    return (
      <div className="flex flex-col justify-center items-center h-[80vh] bg-slate-50 dark:bg-slate-950 px-4 text-center">
        <div className="p-6 bg-rose-50 dark:bg-rose-950/50 border border-rose-200 dark:border-rose-900 rounded-2xl max-w-md shadow-xl">
          ...
          <h2 className="text-lg font-bold text-slate-800 dark:text-slate-100">অ্যাক্সেস ডিনাইড (Access Denied)</h2>
          <p className="text-xs text-slate-500 dark:text-slate-400 mt-2 leading-relaxed">
            আপনার অ্যাকাউন্টটি অথেন্টিকেট করা যায়নি অথবা এর রোল <span className="font-mono text-rose-600 font-bold">admin</span> হিসেবে ভেরিফাই হয়নি। দয়া করে সঠিক অ্যাডমিন অ্যাকাউন্ট দিয়ে লগইন করুন।
          </p>
        </div>
      </div>
    );
    ...
    <div className="p-6 lg:p-8 max-w-7xl mx-auto bg-slate-50 dark:bg-slate-950 min-h-screen text-slate-800 dark:text-slate-100 transition-colors duration-300">
      ...
      <div className="mb-8 flex flex-col md:flex-row md:items-center md:justify-between gap-4 bg-gradient-to-r from-slate-900 via-slate-800 to-rose-950 p-6 rounded-2xl text-white shadow-xl relative overflow-hidden">
        ...
        <div>
          ...
          <h1 className="text-2xl lg:text-3xl font-extrabold tracking-tight text-white">অ্যাডমিন কন্ট্রোল সেন্টার</h1>
          <p className="text-slate-300 text-sm mt-1">ডেটাবেজ থেকে রিয়েল-টাইম ফেচ করা অ্যাডমিন প্রোফাইল তথ্য।</p>
        </div>
      </div>
        ...
        <div className="bg-white dark:bg-slate-900 p-6 rounded-2xl shadow-sm dark:shadow-xl border border-slate-100 dark:border-slate-800 flex flex-col items-center text-center h-fit relative">
          <div className="absolute top-4 right-4">
            <span className="px-2.5 py-1 bg-emerald-50 dark:bg-emerald-950 text-emerald-600 dark:text-emerald-400 text-[10px] font-bold rounded-full border border-emerald-200 dark:border-emerald-900 flex items-center gap-1">
              <span className="w-1.5 h-1.5 rounded-full bg-emerald-500 animate-pulse"></span> Active Admin
            </span>
          </div>

          <div className="relative w-32 h-32 mb-5 mt-2">
            <img 
              src={admin?.image || "https://images.unsplash.com/photo-1534528741775-53994a69daeb?w=400&auto=format&fit=crop&q=80"} 
              alt="Admin Avatar" 
              className="w-full h-full object-cover rounded-full border-4 border-rose-600/30 shadow-lg"
            />
          </div>

          <h2 className="text-xl font-extrabold text-slate-800 dark:text-slate-100">{admin?.name}</h2>
          ...
          <div className="w-full space-y-3.5 text-left text-xs">
            <div className="flex items-center justify-between text-slate-500 dark:text-slate-400 bg-slate-50 dark:bg-slate-950/40 p-2.5 rounded-xl border border-slate-100 dark:border-slate-800/60">
              <span className="flex items-center gap-2"><FiCalendar size={14} className="text-rose-500" /> একাউন্ট তৈরি:</span>
              <span className="font-semibold text-slate-700 dark:text-slate-200">{admin?.createdAt ? new Date(admin.createdAt).toLocaleDateString() : "N/A"}</span>
            </div>
            <div className="flex items-center justify-between text-slate-500 dark:text-slate-400 bg-slate-50 dark:bg-slate-950/40 p-2.5 rounded-xl border border-slate-100 dark:border-slate-800/60">
              <span className="flex items-center gap-2"><FiLock size={14} className="text-rose-500" /> ইমেইল ভেরিফাইড:</span>
              <span className="font-semibold text-emerald-600 dark:text-emerald-400">{admin?.emailVerified ? "Yes" : "No"}</span>
            </div>
          </div>
        </div>
        ...
        <div className="bg-white dark:bg-slate-900 p-6 lg:p-8 rounded-2xl shadow-sm dark:shadow-xl border border-slate-100 dark:border-slate-800 lg:col-span-2 flex flex-col justify-between">
          <div>
            <div className="flex items-center justify-between pb-6 border-b border-slate-100 dark:border-slate-800 mb-6">
              <div>
                <h3 className="text-lg font-bold text-slate-800 dark:text-slate-100">ইউজার কালেকশন অ্যাডমিন ডিটেইলস</h3>
                <p className="text-xs text-slate-400 mt-0.5">ডাটাবেজ থেকে সরাসরি ফেচ করা হয়েছে</p>
              </div>
              <button
              ...
              >
                <FiEdit3 size={14} /> {isEditing ? "এডিট বাতিল" : "প্রোফাইল এডিট করুন"}
              </button>
            </div>
            ...
            {!isEditing ? (
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
                <div className="p-4 bg-slate-50 dark:bg-slate-950/60 rounded-2xl border border-slate-100 dark:border-slate-800/80 shadow-sm">
                  ...
                  <p className="text-sm font-semibold text-slate-800 dark:text-slate-200">{admin?.name}</p>
                </div>

                <div className="p-4 bg-slate-50 dark:bg-slate-950/60 rounded-2xl border border-slate-100 dark:border-slate-800/80 shadow-sm">
                  <span className="text-[11px] font-bold text-slate-400 uppercase tracking-wider flex items-center gap-1.5 mb-1">
                    <FiMail size={14} className="text-rose-500" /> ইমেইল অ্যাড্রেস
                  </span>
                  <p className="text-sm font-semibold text-slate-800 dark:text-slate-200">{admin?.email}</p>
                </div>
                ...
                <div className="p-4 bg-slate-50 dark:bg-slate-950/60 rounded-2xl border border-slate-100 dark:border-slate-800/80 shadow-sm">
                  ...
                  <p className="text-sm font-semibold text-slate-800 dark:text-slate-200">{admin?.phone || "সংযুক্ত নেই"}</p>
                </div>
                ...
                <div className="p-4 bg-slate-50 dark:bg-slate-950/60 rounded-2xl border border-slate-100 dark:border-slate-800/80 shadow-sm">
                  ...
                  <p className="text-sm font-semibold text-slate-800 dark:text-slate-200">{admin?.address || "সংযুক্ত নেই"}</p>
                </div>
              </div>
            ) : (
              <form onSubmit={handleUpdateProfile} className="space-y-4">
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div>
                    <label className="block text-xs font-medium text-slate-500 dark:text-slate-400 mb-1">পূর্ণ নাম</label>
                    <input 
                      type="text" 
                      name="name" 
                      value={formData.name || ""} 
                  ...
                  <div>
                    <label className="block text-xs font-medium text-slate-500 dark:text-slate-400 mb-1">ইমেইল অ্যাড্রেস</label>
                    <input 
                      type="email" 
                      name="email" 
                  ...
                  <div>
                    <label className="block text-xs font-medium text-slate-500 dark:text-slate-400 mb-1">ফোন নম্বর</label>
                    <input 
                      type="text" 
                      name="phone" 
                  ...
                  <div>
                    <label className="block text-xs font-medium text-slate-500 dark:text-slate-400 mb-1">ঠিকানা</label>
                    <input 
                      type="text" 
                      name="address" 
          ...
          <div className="mt-8 pt-4 border-t border-slate-100 dark:border-slate-800 text-right text-xs text-slate-400">
            MongoDB ID: <span className="font-mono text-slate-500">{admin?._id || "N/A"}</span>
          </div>
        </div>
